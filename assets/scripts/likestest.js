...
async function initializeLikesDislikes() {
  ...
  posts.forEach(async (post) => {
      ...
      console.log('Post Likes/Dislikes Data:', data);
  ...
  comments.forEach(async (comment) => {
      ...
      const data = await fetchLikesDislikes(entityId, entityType);
      console.log('Comment Likes/Dislikes Data:', data);
...
async function handleLikeDislike(button, isLike) {
  ...
  try {
      ...
      const parentContainer = button.closest('#post') || button.closest('.comment');
      const likeCountElement = parentContainer.querySelector('.like-count');
      const dislikeCountElement = parentContainer.querySelector('.dislike-count');
      if (likeCountElement) likeCountElement.textContent = data.likes;
      if (dislikeCountElement) dislikeCountElement.textContent = data.dislikes;
